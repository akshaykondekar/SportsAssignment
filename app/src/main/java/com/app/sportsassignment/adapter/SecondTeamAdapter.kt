...
import com.app.sportsassignment.databinding.CvSecondTeamItemBinding
...
//
class SecondTeamAdapter(private val context: Context): RecyclerView.Adapter<SecondTeamAdapter.ViewHolder>() {

    var playerList = emptyList<CustomPlayer>()
    private var itemClickAction: ((player: CustomPlayer) -> Unit)? = null

    inner class ViewHolder(private val binding: CvSecondTeamItemBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bind(player: CustomPlayer) {
            with(binding){
                val playerName = "(${player.position}) ${player.fullName}"
                ...
                itemClickAction?.let { selectedArtist ->
                    cvSecondTeamPlayer.setOnClickListener {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = ViewHolder(
        CvSecondTeamItemBinding.inflate(
